...
from nucleo.models import datos as data_wena
...
def grafico_generico(request, vertiente_id, tipo_grafico):
    ...
    return prueba_2(request,vertiente_id,atributo)

    #return generar_respuesta(request, vertiente_id, atributo, f'dashboard/pruebas_2.html')
...
def prueba_2(request,vertiente_id,atributo):
    ...
    if request.method == 'GET':
            ...
            data = data_wena.objects.filter(fecha__date=hoy, vertiente_id=vertiente_id)
            ...
            return render(request, 'dashboard/pruebas_2.html', context)
            ...
            if fecha_inicio.day==fecha_fin.day:
                ...
                data = data_wena.objects.filter(fecha__date=fecha_inicio, vertiente_id=vertiente_id)
                data_list = list(data.values())  # Convertir los objetos QuerySet a una lista de diccionarios
...
def prueba_1(request):
    ...
    fecha = request.GET.get('fecha', datetime.today().strftime('%Y-%m-%d'))
    ...
    if fecha_inicio and fecha_fin:
        datos = data_wena.objects.filter(fecha__range=(fecha_inicio, fecha_fin))
        datos = datos.annotate(dia=TruncDay('fecha')).values(
            'dia',
            promedio_caudal=Avg('caudal'),
            promedio_ph=Avg('pH'),
            promedio_conductividad=Avg('conductividad'),
            promedio_turbiedad=Avg('turbiedad'),
            promedio_temperatura=Avg('temperatura'),
            promedio_humedad=Avg('humedad')
        ).order_by('dia')
    else:
        datos = data_wena.objects.filter(fecha__date=fecha).annotate(dia=TruncDay('fecha')).values(
            'dia',
            'caudal', 'pH', 'conductividad', 'turbiedad', 'temperatura', 'humedad'
        )
    ...
    for d in datos:
        formatted_data = {
            'dia': DateFormat(d['dia']).format('Y-m-d')  # Formateando la fecha como cadena
        }
        # Agregar promedios o valores individuales según estén disponibles
        for field in ['caudal', 'pH', 'conductividad', 'turbiedad', 'temperatura', 'humedad']:
            if f'promedio_{field}' in d:
                formatted_data[f'promedio_{field}'] = d[f'promedio_{field}']
            elif field in d:
                formatted_data[field] = d[field]
        
        datos_format.append(formatted_data)

    print(datos_format)


    if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
        return JsonResponse({'datos_grafico': json.dumps(datos_format)})
    
    context = {
        'datos_grafico': json.dumps(datos_format)  # Convertir a JSON
    }



    return render(request, 'dashboard/pruebas.html', context)
